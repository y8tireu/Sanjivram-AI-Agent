...
def create_text_generation_pipeline():
    ...
    No local file is required; it will auto-download from Hugging Face by meta.
    """
    if torch.cuda.is_available():
        device_id = 0
        print("GPU is available. Using GPU for faster generation.")
    else:
        device_id = -1
        print("GPU is not available. Using CPU.")

    print("Loading EleutherAI/gpt-neo-2.7B ... This may take some time if CPU is used.")
...
class MainWindow(QMainWindow):
    def __init__(self):
        super().__init__()
        self.setWindowTitle("Local LLM (No Local File) - GPT-Neo 2.7B Demo")
        self.resize(800, 600)
